...
[[resources-client-authenticate]]
=== Authenticating client

A `POST` is used to authenticate Clients.

Notice that if the client is a ClientCompany, the Client will be null, and you can check if it is an admin with the flag
 'admin'.

If the client is not a ClientCompany, then clientCompany will be null, and you're guaranteed that your client is not an
admin.

If both client and clientCompany are null, then the client is not authenticated, and an UNAUTHORIZED will be returned.

==== Example request

include::{snippets}/authenticate-client/curl-request.adoc[]

==== Example response

include::{snippets}/authenticate-client/http-response.adoc[]
